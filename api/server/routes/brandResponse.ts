...
router.post('/:token', async (req: Request, res: Response) => {
  try {
    ...
    try {
      let pushTitle = 'Brand Update';

      let pushBody = `${deal.brand_name || 'A brand'} opened your request.`;

      if (status === 'accepted' || status === 'accepted_verified') {
        pushTitle = '🎉 Offer Accepted!';
        pushBody = `${deal.brand_name || 'A brand'} accepted your offer!`;
      } else if (status === 'negotiating') {
        pushTitle = '💬 Counter Offer';
        pushBody = `${deal.brand_name || 'A brand'} wants to negotiate details.`;
      } else if (status === 'rejected') {
        pushTitle = '❌ Offer Declined';
        pushBody = `${deal.brand_name || 'A brand'} passed on your offer.`;
      }
      ...
      fetch(pushUrl, {
        ...
        body: JSON.stringify({
          ...
          data: { dealId: tokenData.deal_id }
        })
      }).catch(err => console.error('[BrandResponse] Render Push error:', err));
    } catch (e) {
      // ignore
    }
