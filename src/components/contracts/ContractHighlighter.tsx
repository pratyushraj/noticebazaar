...
export const ContractHighlighter: React.FC<ContractHighlighterProps> = ({ contractText, onAskAdvisor }) => {
  ...
  const renderHighlightedText = () => {
    if (flags.length === 0) {
      return <p className="text-foreground/80">{contractText}</p>;
    }
        ...
        {parts.map((part, index) => (
          <span
            ...
            className={cn(
              ...
              part.isFlag && part.flag?.severity === 'high' && "decoration-red-500 bg-destructive/10",
  ...
  return (
    <div className="bg-secondary/[0.06] backdrop-blur-[40px] border border-border rounded-2xl p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <FileText className="w-5 h-5 text-secondary" />
          <h3 className="text-lg font-semibold text-foreground">Contract Review</h3>
        </div>
        {flags.length > 0 && (
          <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-destructive/20 border border-destructive/30">
            <AlertTriangle className="w-4 h-4 text-destructive" />
            <span className="text-sm font-semibold text-destructive">{flags.length} red flag{flags.length !== 1 ? 's' : ''}</span>
          </div>
        )}
      </div>

      <div className="bg-card rounded-xl p-4 mb-4 min-h-[200px] max-h-[400px] overflow-y-auto">
      ...
      {selectedFlag && (
        ...
        >
          <div className="flex items-start justify-between mb-2">
            <div>
              <p className="font-semibold text-foreground mb-1">{selectedFlag.reason}</p>
              <p className="text-sm text-foreground/60">
                Severity: <span className={cn(
                  selectedFlag.severity === 'high' && "text-destructive",
          ...
          <Button
            ...
            className="bg-info hover:bg-info text-foreground text-sm"
            size="sm"
          >
      ...
      {flags.length === 0 && (
        <div className="text-center py-8">
          ...
          <p className="text-foreground/80 font-medium">No red flags detected!</p>
          <p className="text-sm text-foreground/60 mt-1">Your contract looks clean.</p>
        </div>
      )}
    </div>
  );
