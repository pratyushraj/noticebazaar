  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        "bg-secondary/50 backdrop-blur-xl border border-border",
        "shadow-xl shadow-black/10",
        "cursor-pointer transition-all duration-200",
        "hover:bg-secondary/12 hover:border-border/25"
      )}
    >
      {/* Top Row: Amount + Category */}
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full bg-destructive/20 flex items-center justify-center flex-shrink-0 backdrop-blur-md">
      ...
      <div className="space-y-2 text-xs">
        ...
        {expense.vendor_name && (
          <div className="flex items-center gap-2 text-foreground/70">
            <span className="text-foreground/50">Vendor:</span>
        ...
        {expense.tags && expense.tags.length > 0 && (
          <div className="flex items-start gap-2 flex-wrap">
            <span className="text-foreground/50 pt-0.5">Tags:</span>
            <div className="flex flex-wrap gap-1.5">
              {expense.tags.map((tag, index) => (
                <span 
                  key={index}
                  className="px-2 py-0.5 rounded-full bg-secondary/50 text-foreground/80 text-xs"
                >
                  • {tag}
                </span>
              ))}
