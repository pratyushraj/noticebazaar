...
const inferRequestedRole = (
  ...
  const ProtectedRoute = ({ children, allowedRoles, requiredRole }: ProtectedRouteProps) => {
    ...
    const { session, authStatus, profile, refetchProfile, user, isAuthInitializing, isLoadingProfile, isFetchingProfile, isProfileSlow, profileError } = useSession();
  ...
  // Show loader if auth is initializing, session is loading, or profile is loading/fetching
  if (isAuthInitializing || isLoading || ((isLoadingProfile || isFetchingProfile) && !profile && !profileError)) {
    let loaderMessage = "";
    if (isLoadingProfile) {
  ...
  // If loadingProfile or isFetchingProfile is still true, we stay in the loading gate above.
  ...
  if (session && !profile && user && !isLoadingProfile && !isFetchingProfile && (profileError || isProfileSlow)) {
    return (
