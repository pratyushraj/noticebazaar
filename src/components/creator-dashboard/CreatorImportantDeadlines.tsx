...
const CreatorImportantDeadlines: React.FC<CreatorImportantDeadlinesProps> = ({ deadlines, isLoading }) => {
  const getUrgencyBadgeVariant = (urgency: ComplianceDeadline['urgency']) => {
    switch (urgency) {
      case 'High': return 'destructive'; // Red
      case 'Medium': return 'accent'; // Yellowish
    ...
    <Card className="creator-card-base shadow-sm p-6 flex flex-col justify-between min-h-[200px]">
      ...
      <CardContent className="px-0 pb-0 flex-grow">
        {isLoading ? (
          ...
          <p className="text-muted-foreground text-sm py-4">No upcoming deadlines found.</p>
        )}
      </CardContent>
