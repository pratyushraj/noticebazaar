...
export const WelcomeScreen4: React.FC<WelcomeScreen4Props> = ({ onNext, onBack }) => {
  ...
  return (
    <OnboardingSlide>
      ...
      <p className="text-lg md:text-xl font-semibold text-muted-foreground dark:text-foreground/80 mb-6 md:mb-10">
        ...
        {benefits.map((benefit, index) => (
          <GradientCard key={index} padding="sm">
            <div className="flex items-center gap-3">
              ...
              <span className="text-base text-muted-foreground dark:text-foreground/80">{benefit}</span>
            </div>
          </GradientCard>
        ))}
      </div>
