    ...
    <div className="relative">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-white/10 border border-white/20 rounded-xl pl-10 pr-10 py-3 text-white placeholder-white/40 text-[15px] outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-400/20 transition-all"
        />
        {isSearching && (
      ...
      {selectedCreator && (
        <div className="mt-2 flex items-center gap-2 bg-purple-500/20 border border-purple-400/30 rounded-xl px-3 py-2">
          <User className="w-4 h-4 text-purple-300 flex-shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-[13px] text-purple-200 font-medium truncate">
          ...
          <button
            type="button"
            onClick={handleClear}
            className="p-1 hover:bg-purple-400/20 rounded-full flex-shrink-0"
          >
        ...
        <div className="absolute z-50 top-full left-0 right-0 mt-2 bg-[#1a1d2e] border border-white/10 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden">
          {isSearching ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2 text-[13px] text-white/40">Searching creators...</span>
            </div>
          ) : creators.length === 0 ? (
            <div className="py-8 text-center">
              <p className="text-[13px] text-white/40">No creators found for "{debouncedQuery}"</p>
              ...
              {creators.map((creator) => (
                <li key={creator.id}>
                  ...
                  >
                    ...
                    {/* Info */}
                    <div className="flex-1 min-w-0">
                      <p className="text-[14px] text-white font-medium truncate">{creator.name}</p>
      ...
      {/* Hint when empty */}
      {!showResults && !selectedCreator && (
        <p className="mt-2 text-[12px] text-white/25 text-center">
