  ...
  return (
    <div className="min-h-screen bg-[#0D0F1A] text-foreground">
      ...
      <div className="bg-gradient-to-r from-purple-900 to-purple-800 px-4 pt-12 pb-6">
        ...
        <h1 className="text-2xl font-black text-foreground">Send New Offer</h1>
        <p className="text-sm text-foreground/60 mt-1">Find creators and send them collaboration offers</p>
      </div>

      <div className="px-4 py-6 space-y-6">
        {/* Creator Search */}
        <div>
          <h2 className="text-[15px] font-semibold text-foreground/80 mb-3">Step 1 — Find a Creator</h2>
          <CreatorSearch
        ...
        {selectedCreator && (
          <div className="bg-gradient-to-br from-purple-700/30 via-purple-800/20 to-purple-900/10 border border-purple-400/15 rounded-2xl p-5">
            <div className="flex items-start justify-between mb-3">
              <div>
                <h3 className="text-[17px] font-bold text-foreground">{selectedCreator.name}</h3>
                <p className="text-[13px] text-secondary">@{selectedCreator.username}</p>
              </div>
              <CheckCircle2 className="w-5 h-5 text-secondary flex-shrink-0" />
            </div>

            {selectedCreator.bio && (
              <p className="text-[13px] text-foreground/50 mb-3 line-clamp-2">{selectedCreator.bio}</p>
            )}

            {selectedCreator.platforms.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-4">
                {selectedCreator.platforms.map((p) => (
                  <span key={p.name} className="text-[11px] bg-secondary/50 px-2 py-1 rounded-lg text-foreground/60">
                    {p.name}
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-foreground font-semibold py-3 px-4 rounded-xl transition-all text-[15px]"
            >
        ...
        {!selectedCreator && (
          <div className="text-center py-12">
            <div className="w-16 h-16 rounded-full bg-secondary/50 flex items-center justify-center mx-auto mb-4">
              <Send className="w-7 h-7 text-secondary/50" />
            </div>
            <h3 className="text-[16px] font-semibold text-foreground/60 mb-1">Search for a creator above</h3>
