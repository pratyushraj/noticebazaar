...
export default function ReactivationReviews() {
  ...
  return (
    <div className="min-h-full space-y-6 pb-8 text-white relative">
      {/* ── Page Header ── */}
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-xl font-bold tracking-tight">Google Reviews Autoreply</h2>
          <p className="text-sm text-white/40 mt-1">Manage Google Business reviews and automate responses via AI</p>
        </div>
        <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600/10 border border-indigo-500/20">
          <Building2 size={15} className="text-indigo-400" />
          <span className="text-xs font-semibold text-white/80">Smile Dental Clinic (Patna)</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        >
          <div>
            <h3 className="text-sm font-bold text-white mb-1">AI Autoreply Settings</h3>
            <p className="text-[11px] text-white/35">Configure rules for automatically responding to customer reviews</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
            <div>
              <label className="block text-xs font-semibold text-white/40 uppercase tracking-wider mb-1.5">Brand Tone & Voice</label>
              <select
                value={tone}
                onChange={e => setTone(e.target.value as any)}
                className="w-full bg-[#0e1220] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500/60 transition-all"
              >
            ...
            <div>
              <label className="block text-xs font-semibold text-white/40 uppercase tracking-wider mb-1.5">Automation Mode</label>
              <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 h-[42px]">
                <span className="text-xs font-medium text-white/80">Enable Autoreply</span>
                <button
                  onClick={() => setAutoReplyEnabled(!autoReplyEnabled)}
                  className={`relative w-8 h-4.5 rounded-full transition-colors ${autoReplyEnabled ? 'bg-indigo-500' : 'bg-white/10'}`}
                >
        ...
        >
          <div>
            <h3 className="text-sm font-bold text-white mb-1">Performance Overview</h3>
            <p className="text-[11px] text-white/35">Stats from linked Google Profile</p>
          </div>
          <div className="grid grid-cols-2 gap-3 mt-4">
            <div className="bg-white/3 rounded-xl p-3 border border-white/5 text-center">
              <p className="text-lg font-bold text-indigo-400">4.8 ★</p>
              <p className="text-[9px] font-semibold text-white/40 uppercase mt-0.5">Avg Rating</p>
            </div>
            <div className="bg-white/3 rounded-xl p-3 border border-white/5 text-center">
              <p className="text-lg font-bold text-emerald-400">145</p>
              <p className="text-[9px] font-semibold text-white/40 uppercase mt-0.5">Replies Posted</p>
            </div>
          </div>
        </div>
      </div>
      ...
      >
        <div className="px-6 py-4 border-b border-white/[0.05] flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-white">Live Review Feed</h3>
            <p className="text-[11px] text-white/35">Manage and post drafts generated by auto-reply backend</p>
          </div>
        </div>
        ...
        <div className="divide-y divide-white/[0.04]">
          {reviews.map((rev) => {
            ...
            return (
              <div key={rev.id} className="p-6 space-y-4 hover:bg-white/[0.01] transition-all">
                {/* Header info */}
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-white">{rev.commenterName}</h4>
                        ...
                        <div className="flex gap-0.5">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <Star
                              key={i}
                              size={12}
                              className={i < rev.rating ? 'text-yellow-400 fill-yellow-400' : 'text-white/20'}
                            />
                          ))}
                        </div>
                        <span className="text-[10px] text-white/30">• {rev.timeAgo}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    {rev.status === 'replied' && (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase bg-emerald-500/15 text-emerald-400 border border-emerald-500/25">
                        <CheckCircle2 size={11} /> Auto Replied
                      </span>
                    )}
                    {isPending && (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase bg-amber-500/15 text-amber-400 border border-amber-500/25">
                ...
                {/* Review Text */}
                <p className="text-xs text-white/70 italic pl-12 leading-relaxed">
                  "{rev.comment}"
                </p>
                ...
                {rev.aiReplyDraft && (
                  <div className="pl-12">
                    <div className="bg-[#0e1220]/60 border border-indigo-500/20 rounded-xl p-4 space-y-3 relative">
                      <div className="flex items-center justify-between">
                        ...
                        {isPending && !isEditing && (
                          <div className="flex gap-1">
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-3 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-[11px] font-semibold transition-all"
                            >
                              {postingId === rev.id ? (
                        ...
                        {isEditing && (
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-semibold transition-all"
                          >
                            <Check size={11} /> Save
                          </button>
                        )}
                      ...
                      {isEditing ? (
                        <textarea
                          ...
                          className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white leading-relaxed focus:outline-none focus:border-indigo-500/50 outline-none resize-none min-h-[72px]"
                        />
                      ) : (
