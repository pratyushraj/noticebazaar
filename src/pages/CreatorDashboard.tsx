...
const CreatorDashboard = () => {
  ...
  const brandDeals = useMemo(() => {
    return rawBrandDeals.filter(deal => {
      const createdVia = String((deal as any)?.created_via || '');
      // Barter deals can be accepted before delivery details are collected (created via collab request flow).
      // Keep them visible in the dashboard so the creator can proceed to add delivery details.
      if (createdVia !== 'collab_request' && deal.deal_type === 'barter' && (deal.status === 'Drafting' || (deal as any).status === 'drafting') && !deal.delivery_address) {
        return false;
      }
      return true;
    });
