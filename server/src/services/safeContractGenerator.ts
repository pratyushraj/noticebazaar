...
export async function generateSafeContract(
  ...
  return {
    ...
    fileName,
    contentType,
    message: 'Safe contract generated successfully'
  };
}
...
<body>
  ...
  <div class="footer">
    ...
    // Fallback: Use pdf-lib to create a simple PDF
    console.warn('[SafeContractGenerator] Puppeteer unavailable, using pdf-lib fallback:', puppeteerError.message);
    
    try {
      // Try to import pdf-lib (may not be installed)
      ...
      // Header
      page.drawText('Safe Contract Version', {
        x: margin,
        y,
        size: 20,
        font: boldFont,
        color: rgb(0.4, 0.5, 0.92),
      });
      y -= 30;
      ...
      footerPage.drawText('This contract has been reviewed and modified to include creator-friendly clauses.', {
        x: margin,
        y: margin + 20,
        size: 9,
        font,
      ...
      footerPage.drawText('Generated by NoticeBazaar AI Contract Protection Service', {
        x: margin,
        y: margin,
        size: 9,
        font,
      ...
      // Final fallback: Return as text file (user can convert manually)
      console.error('[SafeContractGenerator] pdf-lib also failed:', pdfLibError.message);
      const textContent = `Safe Contract Version\nReport ID: ${reportId}\nGenerated: ${new Date().toLocaleString()}\n\n${'='.repeat(50)}\n\n${text}\n\n${'='.repeat(50)}\n\nThis contract has been reviewed and modified to include creator-friendly clauses.\nGenerated by NoticeBazaar AI Contract Protection Service`;
