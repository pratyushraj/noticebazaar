...
async function main() {
  const email = 'mowgaliaurmalik01@gmail.com';
  const username = 'mowgali_aur_malik';
  const fullName = 'Shalu Malik 🐾';
  const password = 'CreatorArmour2026!'; // Temporary password
  const avatarPublicUrl = 'https://sqqocqujxlgoxbcnfbfb.supabase.co/storage/v1/object/public/creator-assets/mowgali_aur_malik/avatar_perfect_final_1779619876483.png';
  const optimizedVideoPath = path.join(process.cwd(), 'scratch', 'mowgali_optimized.mp4');

  console.log(`🚀 Starting Onboarding & Asset synchronization pipeline for mowgali_aur_malik (@${username})...`);

  try {
    // 1. Verify optimized video exists
    if (!fs.existsSync(optimizedVideoPath)) {
      throw new Error(`Optimized Reel not found at ${optimizedVideoPath}. Run FFmpeg optimization first.`);
    }
    ...
    const profileUpdate = {
      ...
      business_name: 'Shalu 🐥 | mowgali_aur_malik 🐾',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Pet Care & Wholesomeness',
      instagram_followers: 29300,
      followers_count: 29300,
      engagement_rate: 8.3,     // Exact official Meta Interaction Rate from screenshot
      avg_views: 75000,         // Representative view average
      avg_reel_views_manual: 75000,
      reel_price: 10000,         // starts from ₹10,000
      story_price: 3000,         // Dedicated story price ₹3,000
      ...
      city: 'Lucknow',
      bio: '🐾 mowgali_aur_malik | Wholesome dog adventures & funny pet milestones. Managed by Shalu 🐥. Based in Lucknow. 📩 DM for paid campaigns!',
      intro_line: 'Verified Elite Pet Creator from Lucknow with 29.3K followers and 8.3% Engagement Rate 🐾💛',
      collab_intro_line: 'mowgali_aur_malik features high-energy wholesome dog play diaries, gourmet pet treat recipes, and premium dog accessory stories.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),

      // Demographic snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 47.6, men: 52.4 },
      top_cities: ['Delhi', 'Mumbai', 'Lucknow', 'Kolkata'],
      audience_age_range: '18-34 (85.2%)',
      ...
      past_brand_count: 1, // Partnered with 1 brand in the past year
      collab_brands_count_override: 1,

      // Marketplace Intel
      deal_intelligence: {
        hookRate: 68.5, // 68.5% from Marketplace
        interactionRate: 8.3, // 8.3% from Marketplace
        accountsReached30d: '2.1M', // 2.1M from Marketplace
        accountsEngaged30d: '178.8K', // 178.8K from Marketplace
        viralPotential: 'High Partner Activity',
        demographicsRelevance: '98.2% India Concentrated'
      },
      collab_audience_fit_note: 'Superb 98.2% Indian focus with dense concentration of active Gen-Z/Millennial demographic (85.2% 18-34 age group, balanced 47.6% Female / 52.4% Male splits).',
      collab_engagement_confidence_note: 'Sensational 8.3% official Meta interaction rate combined with high 2.1M accounts reached and 178.8K engaged.',
      collab_delivery_reliability_note: 'Lucknow-based pet creator professional, active, and prompt with brief alignment and delivery timelines.',
      collab_cta_trust_note: 'Outstanding recommendation authority for canine food, stylish dog wear, grooming, and smart dog toys.',

      // Custom Standardized Packages compliant with AI_RULES.md
      deal_templates: [
        {
          id: 'starter_collab',
          name: '🚀 Starter Collab',
        ...
        },
        {
          ...
          description: 'Barter collaboration - premium product or experience exchange (minimum value ₹5,000+).',
          deliverables: [
            '1 Reel or 2 Stories',
    ...
    // Phase 3: Link creators and social_accounts
    console.log('[3/8] Linking creators and social accounts tables...');
    ...
    // Phase 4: Upload Optimized Reel
    console.log('[4/8] Uploading optimized vertical Reel to Supabase storage...');
    const fileBuffer = fs.readFileSync(optimizedVideoPath);
    const videoFileName = `discovery-mowgali-${Date.now()}.mp4`;
    ...
    // Phase 5: Onboarding Invite Generation
    console.log('[5/8] Generating secure onboarding recovery invite link...');
    ...
    // Phase 6: Send Onboarding Invitation Email
    console.log('[6/8] Sending professional email invitation via Resend API...');
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Shalu');
    ...
    // Phase 7: Backup and sitemaps sync
    console.log('[7/8] Finalizing local backups and dynamic SEO sitemaps index...');
    try {
