...
async function main() {
  const email = 'simonchoudhary8700@gmail.com';
  const username = 'simon.cchoudhary01';
  const fullName = 'Simon Choudhary';
  const password = 'CreatorArmour2026!'; // Temporary password
  const optimizedVideoPath = path.join(process.cwd(), 'scratch', 'simon_optimized.mp4');

  console.log(`🚀 Starting Onboarding & Asset synchronization pipeline for Simon Choudhary (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'Simon Choudhary | Lifestyle & Fashion curation ✨',
      role: 'creator',
      ...
      reel_price: 35000,     // ₹35,000 paid reel rate
      story_price: 5000,      // Stories rate
      ...
      city: 'Delhi',
      bio: '✨ Lifestyle | Fashion | Travel Curation. Delhi based. Sharing my daily aesthetic outfits, beauty routines, and honest lifestyle vlogs 🌸',
      intro_line: 'Verified Lifestyle & Fashion Creator from Delhi with 56.4K followers 💙',
      collab_intro_line: 'Simon Choudhary features high-engagement fashion lookbooks, aesthetic beauty curation, creative product storytelling, and premium brand collaborations.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),
 
      // Demographic snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 75.0, men: 25.0 }, // 75% Female split
      top_cities: ['Delhi', 'Bangalore', 'Mumbai', 'Kolkata'],
      audience_age_range: '18-24 (68.6%)',
      ...
      // Marketplace Intel
      deal_intelligence: {
        hookRate: 60.4,       // Extremely strong 60.4% hook rate!
        interactionRate: 1.9,
        accountsReached30d: '7.9M', // 7.9M Reach
        accountsEngaged30d: '241K', // 241K Engaged
        viralPotential: 'High',
        demographicsRelevance: '90.6% India Concentrated'
      },
      collab_audience_fit_note: 'Excellent 75.0% Female engaged audience concentration—highly optimal for premium Indian fashion labels, cosmetic/skincare brands, and premium lifestyle collections.',
      collab_engagement_confidence_note: 'High-impact fashion aesthetics combined with an exceptionally high 60.4% Hook Rate and solid 7.9M monthly reach.',
      collab_delivery_reliability_note: 'Delhi-based lifestyle creator with professional DM responses, very high execution standards, and rich curation.',
      collab_cta_trust_note: 'Excellent CTR and conversions for visual fashion accessories, cosmetic routines, wellness products, and modern daily aesthetics.',

      // Custom Standardized Packages (Starting from 35k)
      deal_templates: [
        {
          id: 'starter_collab',
          name: '🚀 Starter Collab',
    ...
    const videoFileName = `discovery-simon-${Date.now()}.mp4`;
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Simon');
