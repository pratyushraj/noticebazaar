...
async function main() {
  const email = 'kumarsinghsaurav262@gmail.com';
  const username = 'sauravsights';
  const fullName = 'Saurav Kumar Singh';
  const password = 'CreatorArmourSaurav2026!'; // Temporary password
  const optimizedVideoPath = path.join(process.cwd(), 'scratch', 'sauravsights_optimized.mp4');

  console.log(`🚀 Starting Onboarding & Asset synchronization pipeline for Saurav Kumar Singh (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'Saurav Kumar Singh | Travel, Street Photography & Experience Curation ✨',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Travel & Lifestyle',
      instagram_followers: 6929,
      followers_count: 6929,
      engagement_rate: 11.7, // Phenonemal 11.7% Interaction/Engagement Rate!
      avg_views: 80000,
      avg_reel_views_manual: 80000,
      reel_price: 4000,      // ₹4,000 paid reel rate
      story_price: 1500,      // Stories rate
      starting_price: 4000,
      barter_min_value: 3000, // Barter value minimum of ₹3,000
      open_to_collabs: true,
      collaboration_preference: 'both',
      is_verified: true,
      is_elite_verified: true,
      location: 'Delhi NCR / Patna, India',
      city: 'Delhi',
      bio: '✨ Visual Storyteller | Street & Travel Photographer | Delhi NCR & Patna base. Capturing authentic street aesthetic, travel reviews, and local culinary experiences 📸',
      intro_line: 'Verified Travel & Photography Creator from Delhi NCR / Patna with 6.9K followers and 11.7% Interaction Rate 📸✨',
      collab_intro_line: 'Saurav Kumar Singh features high-aesthetic street photography vlogs, visual travel storytelling, experiential brand showcases, and creative visual collaborations.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),
 
      // Demographic snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 48.1, men: 51.9 }, // 48.1% Female, 51.9% Male
      top_cities: ['Delhi', 'Mumbai', 'Ahmedabad', 'Kolkata', 'Patna'],
      audience_age_range: '18-24 (55.8%)',
      ...
      // Marketplace Intel
      deal_intelligence: {
        hookRate: 38.6,       // Strong 38.6% hook rate
        interactionRate: 11.7, // Phenomenal 11.7% interaction rate!
        accountsReached30d: '2.6M', // 2.6M Reach
        accountsEngaged30d: '431.7K', // 431.7K Engaged
        viralPotential: 'High',
        demographicsRelevance: '94.1% India Concentrated'
      },
      collab_audience_fit_note: 'A highly balanced gender split (48% female, 52% male) concentrated in major metros and Patna—perfectly suited for travel equipment, tech lifestyle, local experience launches, cafe reviews, and streetwear labels.',
      collab_engagement_confidence_note: 'Staggering 11.7% official Meta interaction rate coupled with 2.6M monthly reach and 431.7K highly engaged accounts.',
      collab_delivery_reliability_note: 'Extremely responsive street and travel photographer with high aesthetic standards and custom audio-visual narrative style.',
      collab_cta_trust_note: 'High viral potential and high-converting click-throughs for outdoor dining reviews, travel hotspots, street-culture collections, and local events.',
      ...
      deal_templates: [
        ...
        },
        {
          ...
          description: 'Barter collaboration - premium product or experience exchange (minimum value ₹3,000+).',
          deliverables: [
            '1 Reel or 2 Stories',
    ...
    const videoFileName = `discovery-sauravsights-${Date.now()}.mp4`;
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Saurav');
