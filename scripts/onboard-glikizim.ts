...
async function main() {
  const email = 'glikizim01@gmail.com';
  const username = 'indian_gli_kizim';
  const fullName = 'Gli_Kizim 🇮🇳';
  const password = 'CreatorArmour2026!'; // Temporary password
  const avatarPublicUrl = 'https://sqqocqujxlgoxbcnfbfb.supabase.co/storage/v1/object/public/creator-assets/indian_gli_kizim/avatar_perfect_final_1779618616475.png';
  const optimizedVideoPath = path.join(process.cwd(), 'scratch', 'glikizim_optimized.mp4');

  console.log(`🚀 Starting Onboarding & Asset synchronization pipeline for Gli_Kizim (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'Gli_Kizim 🇮🇳 | Wholesome Pet & Cat Stories 🐾😻',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Pet Care & Wholesomeness',
      instagram_followers: 56600,
      followers_count: 56600,
      engagement_rate: 6.2,     // Exact official Meta Interaction Rate from screenshot
      avg_views: 185000,         // Representative view average for 4.3M reach and dense active community
      avg_reel_views_manual: 185000,
      reel_price: 6000,          // starts from ₹6,000
      story_price: 2000,         // dedicated story set ₹1,500-₹3,000, so ₹2,000 is mid-range perfect
      starting_price: 6000,
      ...
      city: 'Bengaluru',
      bio: '🐾 Wholesome cat antics & daily pawsome diaries of Gli_Kizim! Managed by hoomans. Based in Bengaluru. 📩 Open for brand collabs & campaigns!',
      intro_line: 'Verified Elite Pet Creator from Bengaluru with 56.6K followers and 6.2% Engagement Rate 🐾😻',
      collab_intro_line: 'Gli_Kizim features wholesome daily cat diaries, aesthetic pet toy check-ins, and high-quality pet care reviews.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),

      // Demographic snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 82.0, men: 18.0 },
      top_cities: ['Bengaluru', 'Hyderabad', 'Mumbai', 'Delhi'],
      audience_age_range: '18-34 (89.1%)',
      ...
      deal_intelligence: {
        ...
        interactionRate: 6.2, // 6.2% from Marketplace
        accountsReached30d: '4.3M', // 4.3M from Marketplace
        accountsEngaged30d: '281.8K', // 281.8K from Marketplace
        viralPotential: 'Very High',
        demographicsRelevance: '94.5% India Concentrated'
      },
      collab_audience_fit_note: 'Strong 94.5% Indian focus with dense concentration of premium female pet parent demographic (82.0% Female, 89.1% 18-34 age group).',
      collab_engagement_confidence_note: 'Outstanding 6.2% official Meta interaction rate combined with high 4.3M accounts reached and 281.8K engaged.',
      collab_delivery_reliability_note: 'Bengaluru-based pet creator professional and prompt with brief alignment and delivery timelines.',
      collab_cta_trust_note: 'High conversions for pet food, gourmet treats, cat litter, feline toys, stylish pet apparel, and smart home pet accessories.',
    ...
    const videoFileName = `discovery-glikizim-${Date.now()}.mp4`;
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Gli_Kizim');
