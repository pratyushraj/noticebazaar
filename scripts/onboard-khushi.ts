...
async function main() {
  const email = 'nagatithi@gmail.com';
  const username = 'capture.by.khushi';
  const fullName = 'Atithi Nag';
  const password = 'CreatorArmour2026!'; // Temporary password
  const avatarPublicUrl = 'https://sqqocqujxlgoxbcnfbfb.supabase.co/storage/v1/object/public/creator-assets/capture.by.khushi/avatar_perfect_final_1779635466362.png';
  const optimizedVideoPath = path.join(process.cwd(), 'scratch', 'khushi_optimized.mp4');

  console.log(`🚀 Starting Onboarding & Asset synchronization pipeline for Khushi (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'capture.by.khushi 🎀',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Beauty & Lifestyle',
      instagram_followers: 27200,
      followers_count: 27200,
      engagement_rate: 6.0,     // Exact official Meta Interaction Rate from screenshot
      avg_views: 12000,         // Conservative baseline views estimation
      ...
      city: 'Lucknow',
      bio: '🎀 Sweet girl but will roast her own pimples if needed ♡ 🍓 Serving trials, errors & everyday glow-ups 🤷‍♀️ Beauty | Skincare | Fashion | Food',
      intro_line: 'Verified Elite Beauty & Lifestyle Creator from Lucknow with 27K followers and 6% Interaction Rate 🎀✨',
      collab_intro_line: 'Atithi Nag (capture.by.khushi) features aesthetic skincare trials, honest beauty glow-ups, fashion lookbooks, and daily lifestyle updates.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),

      // Demographic snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 86.8, men: 13.2 },
      top_cities: ['Delhi', 'Lucknow', 'Mumbai', 'Chennai'],
      audience_age_range: '18-24 (78.3%)',
      ...
      past_brand_count: 7,
      collab_brands_count_override: 7,
      past_brands: ['Tira', 'Indulekha', 'Sunsilk', 'Dove', 'Kaya', 'TRESemmé', "McDonald's"],

      // Marketplace Intel
      deal_intelligence: {
        hookRate: 51.5,
        interactionRate: 6.0,
        accountsReached30d: '60.8K',
        accountsEngaged30d: '3.4K',
        viralPotential: 'High',
        demographicsRelevance: '95.8% India Concentrated'
      },
      collab_audience_fit_note: 'High 95.8% Indian concentration with heavy density in Lucknow (2.7%) and major metros (Delhi 4.2%). Highly relevant female-centric demographic (86.8% female).',
      collab_engagement_confidence_note: 'Excellent 6% Meta interaction rate, significantly outperforming average benchmarks, showing active visual hook retention.',
      collab_delivery_reliability_note: 'Highly engaging Lucknow-based creator with past experience collaborating with prominent brands like Tira, Sunsilk, Dove, and TRESemmé.',
      collab_cta_trust_note: 'Outstanding conversion potential for premium and mid-tier D2C beauty, skincare, haircare, and lifestyle brands seeking highly authentic, relatable trials.',
      ...
      deal_templates: [
        ...
        },
        {
          ...
          rate: 0,
          barter_min_value: 10000,
          description: 'Barter collaboration - depends on brand & product positioning (minimum value ₹10,000+).',
          deliverables: [
            '1 Reel or 2 Stories',
    ...
    const videoFileName = `discovery-khushi-${Date.now()}.mp4`;
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Khushi');
