...
async function main() {
  const email = 'bihagirlofficial@gmail.com';
  const username = 'biharigirl_anjaliagarwal';
  const fullName = 'Anjali Agarwal | Biharigirl 🌸';
  const password = 'CreatorArmour2026!';
  const reelUrl = 'https://www.instagram.com/reels/C--VP9EJBC8/';

  console.log(`🚀 Starting Full Onboarding & Discovery Video Asset pipeline for Anjali (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'Anjali Agarwal 🌸',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Fashion & Beauty',
      instagram_followers: 106500,
      followers_count: 106500,
      engagement_rate: 4.8,
      avg_views: 45000,
      avg_likes_manual: 3200,
      ...
      city: 'Patna',
      phone: '8294488533',
      bio: 'Lifestyle/Beauty | Fashion Managed by @houseofcre... | Patna, India 📍',
      intro_line: 'Verified Fashion & Beauty Creator from Patna with 106K followers and 4.8% engagement 🌸✨',
      collab_intro_line: 'Anjali Agarwal showcases stunning fashion lookbooks, beauty tutorials, makeup transitions, and lifestyle vlogs.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),

      // Audience Demographics Snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 80.6, men: 19.4 },
      top_cities: ['Delhi', 'Kolkata', 'Ahmedabad', 'Mumbai'],
      audience_age_range: '18-24 (58.6%)',
      ...
      deal_intelligence: {
        hookRate: 75.8,
        interactionRate: 4.8,
        accountsReached30d: '92K',
        accountsEngaged30d: '12.8K',
        viralPotential: 'Strong',
        demographicsRelevance: '92.5% India Concentrated'
      },
      collab_audience_fit_note: 'Excellent 80.6% female audience skew, concentrated heavily in metro cities and youth segments.',
      collab_engagement_confidence_note: 'Strong 4.8% interaction rate backed by an impressive 75.8% hook rate, showing high dynamic retention on fashion lookbooks and reels.',
      collab_delivery_reliability_note: 'Consistent reach with 92K+ active monthly accounts and dedicated engagement from over 12.8K monthly accounts.',
      collab_cta_trust_note: 'Ideal conversion partner for D2C cosmetics, clothing labels, hair accessories, luxury styling barter, and lifestyle experiences.',
      ...
      deal_templates: [
        {
          ...
          price: 20000,
          budget: 20000,
          description: '1 Collab Reel (15-30s) featuring Anjali - High hook retention, dynamic beauty/fashion transition, and premium product integration.',
          deliverables: [
            '1 Collab Reel (15-30s) featuring product',
            'High hook retention layout focus',
            '1 Revision included'
          ]
        },
        {
          ...
          budget: 25000,
          isPopular: true,
          description: '1 Premium Reel + 1 Story Reshare - Excellent for direct brand conversions, swipe-up link tracking, and wider distribution.',
          deliverables: [
            '1 Premium Collab Reel (30-60s)',
            '30-day organic usage rights',
            '1 Story Reshare with link sticker',
            '1 Revision included'
          ]
        },
        {
          ...
          description: 'Barter collaboration - Only accepting premium styling apparel, skincare/cosmetics hampers, or luxury accessories (minimum value ₹8,000+).',
          deliverables: [
            '1 Showcase Reel or 2 Interactive Stories',
            'Product styling review focus'
          ]
        }
      ]
    };
    ...
    const rawFileName = `raw-anjali-${Date.now()}.mp4`;
    const rawFilePath = path.join(tempDir, rawFileName);
    
    const optFileName = `discovery-anjali-${Date.now()}.mp4`;
    ...
    // Phase 6: Re-encode video with FFmpeg for Safari/iOS compatibility (CRF 28 to preserve size limits)
    console.log(`[6/8] Re-encoding video with FFmpeg for Safari/iOS compatibility (H.264 vertical 720p format)...`);
    try {
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Anjali');
    ...
    console.log(`\n✨ Public onboarding profile and dynamic storefront is now active for ${fullName}!`);
