...
async function main() {
  const email = 'anjalisingh29jan2001@gmail.com';
  const username = 'styleanddine_29';
  const fullName = 'Anjali Singh';
  const password = 'CreatorArmourAnjaliS2026!'; // Temporary password
  const optimizedVideoPath = path.join(process.cwd(), 'scratch', 'styleanddine_optimized.mp4');

  console.log(`🚀 Starting Onboarding & Asset synchronization pipeline for Anjali Singh (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'Anjali Singh | Food, Travel & Property Model ✨🍽️',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Travel & Lifestyle',
      instagram_followers: 14700,
      followers_count: 14700,
      engagement_rate: 4.3, // Official Meta Interaction Rate from screenshot
      avg_views: 45000,
      avg_reel_views_manual: 45000,
      reel_price: 5000,      // ₹5,000 paid starting rate
      ...
      city: 'Ranchi',
      bio: 'Food & Travel Creator 🧿 Ranchi based. Lifestyle, Fashion & Property Model sharing professional cafe vlogs, culinary reviews, and scenic travel diaries ✨',
      intro_line: 'Verified Food, Travel & Lifestyle Creator from Ranchi with 14.7K followers and 4.3% Interaction Rate 📸✨',
      collab_intro_line: 'Anjali Singh showcases professional cafe vlogs, culinary reviews, aesthetic fashion lookbooks, and scenic travel diaries.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),
 
      // Demographic snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 54.5, men: 45.5 }, // Perfect balanced split!
      top_cities: ['Ranchi', 'Delhi', 'Jamshedpur', 'Patna'],
      audience_age_range: '25-34 (45.6%)',
      ...
      // Marketplace Intel
      deal_intelligence: {
        hookRate: 47.5,       // Strong 47.5% hook rate from screenshot
        interactionRate: 4.3,
        accountsReached30d: '484.4K', // From screenshot
        accountsEngaged30d: '42.8K', // From screenshot
        viralPotential: 'Medium',
        demographicsRelevance: '99% India Concentrated, balanced gender split'
      },
      collab_audience_fit_note: 'Outstanding 54.5% Female / 45.5% Male balanced audience split concentrated in Ranchi and Jamshedpur—uniquely optimal for local cafe launches, dining sponsorships, travel gear, lifestyle cosmetics, and property promotions.',
      collab_engagement_confidence_note: 'Strong visual storytelling combined with a solid 4.3% Interaction Rate and 42.8K monthly engaged accounts.',
      collab_delivery_reliability_note: 'Ranchi-based food and travel creator with highly professional property lookbooks and rich culinary curation.',
      collab_cta_trust_note: 'Excellent CTR and conversion potential for outdoor cafes, travel products, staycations, cosmetics, and lifestyle items.',
    ...
    const videoFileName = `discovery-styleanddine-${Date.now()}.mp4`;
