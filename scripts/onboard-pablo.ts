...
async function main() {
  const email = 'pabloescobarkgolden@gmail.com';
  const username = 'golden_pabloescobark';
  const fullName = 'Pablo | The Golden Child 🐶';
  const password = 'CreatorArmour2026!'; // Temporary credential
  const reelUrl = 'https://www.instagram.com/reel/DYkCJ46hh7f/?igsh=YXdxcWtrZTltOTJr';

  console.log(`🚀 Starting Full Onboarding & Discovery Video Asset pipeline for Pablo (@${username})...`);
  ...
  try {
    ...
    const pastBrands = ['Bark Licking Pet Food'];
    const brandLogos = ['https://logo.clearbit.com/barklicking.com'];
    ...
    const profileUpdate = {
      ...
      business_name: 'Pablo | The Golden Child 🐾',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Pet Care & Wholesomeness',
      instagram_followers: 85300,
      followers_count: 85300,
      engagement_rate: 10.6, // Exact official Meta Interaction Rate (High!)
      avg_views: 337000,    // Exact official Meta Accounts Engaged (30d: 337.0K)
      avg_reel_views_manual: 337000,
      reel_price: 20000,   // From DM commercial rate
      story_price: 4000,    // Estimated standard
      starting_price: 20000,
      open_to_collabs: true,
      collaboration_preference: 'paid', // "Currently we are not taking any barter collaborations."
      is_verified: true,
      is_elite_verified: true,
      location: 'Karnataka, India',
      city: 'Bangalore',
      bio: '🐾 Golden Boy spreadin wholesome puppy energy everywhere! Loves long hikes, organic dog food, and posing in stylish custom waistcoats. Managed by hoomans.',
      intro_line: 'Premium Golden Retriever from Bangalore with 85.3K followers and 10.6% Interaction Rate 🐾💛',
      collab_intro_line: 'Pablo the Golden Child features wholesome daily antics, high-energy outdoor excursions, and premium dog accessory stories.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),

      // Audience Demographics Snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 59.9, men: 40.1 },
      top_cities: ['Bangalore', 'Delhi', 'Kolkata', 'Mumbai'],
      audience_age_range: '18-34 (86.9%)',
      primary_audience_language: 'English / Hindi',

      // System Trust Signals
      deal_score: 97, // High score due to excellent interaction rate & hook rate
      ...
      // Marketplace Intel
      deal_intelligence: {
        hookRate: 61.7, // 61.7% from Marketplace (Incredible!)
        interactionRate: 10.6, // 10.6% from Marketplace
        accountsReached30d: '2.5M', // 2.5M from Marketplace
        accountsEngaged30d: '337K', // 337K from Marketplace
        viralPotential: 'Top 5% Partner Activity',
        demographicsRelevance: '92.9% India Concentrated'
      },
      collab_audience_fit_note: 'Exceptional 92.9% Indian concentration with heavy density in Bangalore (11.5%) and major metros.',
      collab_engagement_confidence_note: 'Industry-leading 10.6% interaction rate combined with an elite 61.7% video hook retention rate.',
      ...
      deal_templates: [
        {
          ...
          budget: 20000,
          isPopular: true,
          description: '1 Wholesome Collab Reel featuring Pablo - Perfect for brand awareness and active organic reach.',
          deliverables: [
            '1 Reel (15-30s) featuring Pablo the Golden Child',
            'Full organic rights',
        ...
        },
        {
          id: 'premium_package',
          type: 'paid',
          label: '⭐ Golden Growth Campaign',
          budget: 20000, // Matching the commercial package in DM
          isPopular: false,
          description: '1 Wholesome Reel + 2 Organic Stories - Best for driving high conversions and traffic.',
          deliverables: [
            '1 Collab Reel (15-30s) with Pablo',
    ...
    const rawFileName = `raw-pablo-${Date.now()}.mp4`;
    const rawFilePath = path.join(tempDir, rawFileName);
    
    const optFileName = `discovery-pablo-${Date.now()}.mp4`;
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Pablo');
