...
async function main() {
  const email = 'aneesshumyla@gmail.com';
  const username = 'shumylaa_anees';
  const fullName = 'Shumyla';
  const password = 'CreatorArmour2026!'; // Temporary password
  const optimizedVideoPath = path.join(process.cwd(), 'scratch', 'shumyla_optimized.mp4');

  console.log(`🚀 Starting Onboarding & Asset synchronization pipeline for Shumyla (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'Shumyla | Skincare, Fashion & Beauty 💕',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Fashion & Beauty',
      instagram_followers: 25100,
      followers_count: 25100,
      engagement_rate: 4.5,
      avg_views: 35000,
      avg_reel_views_manual: 35000,
      reel_price: 6000,     // ₹6,000 starting reel rate
      ...
      city: 'Noida',
      bio: '💕 Skincare | Fashion | Beauty | Lifestyle curation. Based in Noida. Sharing my honest skincare routines, aesthetic fashion styling, and daily self-care lifestyle ✨',
      intro_line: 'Verified Skincare & Fashion/Beauty Creator from Noida with 25.1K followers 💕',
      collab_intro_line: 'Shumyla\'s World features authentic skincare reviews, daily fashion aesthetics, self-care lifestyle curation, and creative product partnerships.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),

      // Demographic snapshot
      audience_gender_split: { women: 85.0, men: 15.0 }, // High Female split!
      top_cities: ['Noida', 'Delhi', 'Gurugram', 'Ghaziabad'],
      audience_age_range: '18-24 (45.0%)',
      ...
      deal_intelligence: {
        hookRate: 40.0,
        interactionRate: 4.5,
        accountsReached30d: '200K',
        accountsEngaged30d: '8K',
        viralPotential: 'Medium',
        demographicsRelevance: '95% India Concentrated'
      },
      collab_audience_fit_note: 'Fantastic 85.0% Female audience split—highly optimal for daily skincare, cosmetics, wellness, and self-care brands.',
      collab_engagement_confidence_note: 'Highly aesthetic lifestyle curation paired with solid engagement on skincare and fashion catalog.',
      collab_delivery_reliability_note: 'Noida-based lifestyle creator prompt with brief execution and clean product styling.',
      collab_cta_trust_note: 'Excellent conversions for skincare treatments, cosmetics, wellness products, and aesthetic local accessories.',

      // Custom Standardized Packages (Starting from 6k)
      deal_templates: [
        {
          id: 'starter_collab',
          name: '🚀 Starter Collab',
        ...
        },
        {
          ...
          description: 'Barter collaboration - premium product or experience exchange (minimum value ₹4,000+).',
          deliverables: [
            '1 Reel or 2 Stories',
    ...
    const videoFileName = `discovery-shumyla-${Date.now()}.mp4`;
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Shumyla');
