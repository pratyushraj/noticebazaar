...
async function main() {
  const email = 'parulsingh112003@gmail.com';
  const username = 'parulsinghrajputt';
  const fullName = 'Parul Singh';
  const password = 'CreatorArmourParulS2026!'; // Temporary password
  const optimizedVideoPath = path.join(process.cwd(), 'scratch', 'parulsingh_optimized.mp4');

  console.log(`🚀 Starting Onboarding & Asset synchronization pipeline for Parul Singh (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'Parul Singh | Fashion, Beauty & Lifestyle Creator 🌸✨',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Fashion & Lifestyle',
      instagram_followers: 6372,
      followers_count: 6372,
      engagement_rate: 6.5, // Official Meta Interaction Rate from screenshot
      avg_views: 35000,
      avg_reel_views_manual: 35000,
      reel_price: 4000,      // ₹4,000 paid starting rate
      story_price: 1200,      // Stories rate
      starting_price: 4000,
      barter_min_value: 2500, // Barter minimum value from DM responses
      ...
      city: 'Patna',
      bio: 'Fashion | Beauty | Lifestyle 🤍 Patna / Delhi. Just a girl & her crossroads sharing aesthetic local guides, styling vlogs, and beauty reviews.',
      intro_line: 'Verified Fashion, Beauty & Lifestyle Creator from Patna with 6.3K followers and exceptional 6.5% Interaction Rate 📸✨',
      collab_intro_line: 'Parul Singh crafts highly engaged aesthetic styling vlogs, local destination guides, and relatable beauty reviews.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),
 
      // Demographic snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 54.0, men: 46.0 }, // Balanced gender split
      top_cities: ['Delhi', 'Mumbai', 'Patna', 'Lucknow'],
      audience_age_range: '18-24 (46.5%)',
      ...
      // Marketplace Intel
      deal_intelligence: {
        hookRate: 57.5,       // Strong 57.5% hook rate from screenshot
        interactionRate: 6.5,
        accountsReached30d: '1.9M', // From screenshot
        accountsEngaged30d: '219.6K', // From screenshot
        viralPotential: 'High',
        demographicsRelevance: '99% India Concentrated, highly active youth demographic'
      },
      collab_audience_fit_note: 'Outstanding 54.0% Female / 46.0% Male balanced audience split concentrated heavily in Delhi and Patna—highly active youth audience (18-24 age tier representing 46.5%) perfectly optimized for cosmetics, styling lookbooks, cafe openings, and dynamic local hotel campaigns.',
      collab_engagement_confidence_note: 'Exceptional 6.5% Interaction Rate combined with 219.6K monthly engaged accounts showing incredible community trust and content resonance.',
      collab_delivery_reliability_note: 'Patna & Delhi-based food, fashion, and lifestyle creator with highly creative video styling and rich visual aesthetics.',
      collab_cta_trust_note: 'Very high click-through rate and product engagement potential, particularly across Gen Z fashion brands, cafes, beauty gear, and local staycations.',
      ...
      deal_templates: [
        ...
        },
        {
          ...
          budget: 0,
          rate: 0,
          barter_min_value: 2500,
          description: 'Barter collaboration - premium product or experience exchange (minimum value ₹2,500+).',
          deliverables: [
            '1 Reel or 2 Stories',
    ...
    const videoFileName = `discovery-parulsingh-${Date.now()}.mp4`;
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Parul');
