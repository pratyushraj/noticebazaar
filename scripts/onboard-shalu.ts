...
async function main() {
  const email = 'shaluofficial001@gmail.com';
  const username = 'shalu.aestheticvlog';
  const fullName = 'Shalu Mehta';
  const password = 'CreatorArmourShaluM2026!'; // Temporary password
  const optimizedVideoPath = path.join(process.cwd(), 'scratch', 'shalu_optimized.mp4');

  console.log(`🚀 Starting Onboarding & Asset synchronization pipeline for Shalu Mehta (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'Shalu Mehta | Aesthetic Content Creator 📸✨',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Travel & Lifestyle',
      instagram_followers: 2888,
      followers_count: 2888,
      engagement_rate: 5.6, // Official Meta Interaction Rate from screenshot
      avg_views: 18000,
      avg_reel_views_manual: 18000,
      reel_price: 2500,      // ₹2,500 highly competitive paid starting rate from DM responses
      story_price: 800,       // Stories rate
      starting_price: 2500,
      barter_min_value: 3000, // Barter minimum value (3k to 5k)
      ...
      city: 'Patna',
      bio: 'Aesthetic Content Creator 🧿 Patna / Purnea based. Vlogging daily lifestyle snippets, cinematic cafe reviews, and minimalist fashion styling lookbooks ✨🎬',
      intro_line: 'Verified Aesthetic & Travel Creator from Patna with 2.8K followers and highly active 5.6% Interaction Rate 📸✨',
      collab_intro_line: 'Shalu Mehta constructs beautifully graded aesthetic daily vlogs, cinematic cafe walkthroughs, and minimal product showcases.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),
 
      // Demographic snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 41.5, men: 58.5 }, // Strong male/female split
      top_cities: ['Patna', 'Delhi', 'Purnia', 'Pune'],
      audience_age_range: '18-24 (54.7%)',
      ...
      // Marketplace Intel
      deal_intelligence: {
        hookRate: 53.9,       // Strong 53.9% hook rate from screenshot
        interactionRate: 5.6,
        accountsReached30d: '573.5K', // Massively viral for sub-3k followers!
        accountsEngaged30d: '46.6K', // Strong engagement
        viralPotential: 'High',
        demographicsRelevance: '99% India Concentrated, extremely strong Patna & Bihar density'
      },
      collab_audience_fit_note: 'Remarkable 21.2% Patna localized audience density concentrated heavily in the 18-24 youth demographic (representing 54.7%)—uniquely tailored for local Patna boutique openings, Boring Road salon expansions, dining reviews, and aesthetic product styling.',
      collab_engagement_confidence_note: 'Stellar visual aesthetics backed by a solid 5.6% Interaction Rate and an incredible 573.5K accounts reached monthly.',
      collab_delivery_reliability_note: 'Patna-based daily vlogger and aesthetic creator with high post consistency and professional visual color grading.',
      collab_cta_trust_note: 'Very high click-through rate and product engagement potential for Gen Z local products, skincare Detans, haircuts, staycations, and dining experiences.',
      ...
      deal_templates: [
        ...
        },
        {
          ...
          description: 'Barter collaboration - premium product or experience exchange (minimum value ₹3,000 to ₹5,000).',
          deliverables: [
            '1 Reel or 2 Stories',
    ...
    const videoFileName = `discovery-shalu-${Date.now()}.mp4`;
