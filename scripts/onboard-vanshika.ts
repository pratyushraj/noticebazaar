...
async function main() {
  const email = 'vanshiiii2004@gmail.com';
  const username = 'badiekisadielife_';
  const fullName = 'Vanshika Rawat';
  const phone = '8595026856';
  const password = 'CreatorArmour2026!'; // Temporary password
  const optimizedVideoPath = path.join(process.cwd(), 'scratch', 'vanshika_optimized.mp4');

  console.log(`🚀 Starting Onboarding & Asset synchronization pipeline for Vanshika Rawat (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'Vanshika Rawat | Lifestyle, Fashion & Beauty ✨',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Fashion & Beauty',
      instagram_followers: 33100,
      followers_count: 33100,
      engagement_rate: 6.1,  // High 6.1% engagement rate!
      avg_views: 45000,
      avg_reel_views_manual: 45000,
      reel_price: 8000,      // ₹8,000 paid reel rate
      story_price: 2000,      // Stories rate
      starting_price: 8000,
      ...
      city: 'Noida',
      bio: '✨ UGC Creator | Lifestyle | Fashion | Food & Travel 📍 Noida based. Capturing aesthetic routines, self-care styling, sensory reviews, and premium lifestyle narratives 🌸',
      intro_line: 'Verified Lifestyle & Fashion Creator from Noida with 33.1K followers 🧡',
      collab_intro_line: 'Vanshika Rawat features high-engagement UGC styling, aesthetic daily routines, skincare/beauty transformations, and creative visual collaborations.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),
 
      // Demographic snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 72.0, men: 28.0 }, // 72% Female split
      top_cities: ['Delhi', 'Noida', 'Ghaziabad', 'Greater Noida'],
      audience_age_range: '25-34 (48.1%)',
      ...
      past_brand_count: 6,     // 6 verified past brands!
      collab_brands_count_override: 6,
      past_brands: ['Be Bodywise', 'Naturali', "L'Oréal Professionnel", 'Asaya', 'Dot & Key', 'Jovees Herbal'],

      // Marketplace Intel
      deal_intelligence: {
        hookRate: 47.4,       // Strong 47.4% hook rate
        interactionRate: 6.1, // Outstanding 6.1% interaction rate!
        accountsReached30d: '1M',
        accountsEngaged30d: '82.4K',
        viralPotential: 'Medium-High',
        demographicsRelevance: '99.1% India Concentrated'
      },
      collab_audience_fit_note: 'Outstanding 72.0% Female engaged audience concentration—highly optimal for premium Indian fashion labels, cosmetic/skincare brands, and daily personal wellness categories.',
      collab_engagement_confidence_note: 'Strong visual styling combined with a very high 6.1% Interaction Rate and 82.4K monthly engaged users.',
      collab_delivery_reliability_note: 'Noida-based visual and UGC creator with exceptionally swift communications, brief execution, and rich sensory curation.',
      collab_cta_trust_note: 'Strong click-throughs and conversions for daily hair-care treatments, skincare, aesthetic cosmetic collections, and wellness packages.',

      // Custom Standardized Packages (Starting from 8k)
      deal_templates: [
        {
          id: 'starter_collab',
          name: '🚀 Starter Collab',
    ...
    console.log('   ✅ Creators table link established with verified contact number.');
    ...
    const videoFileName = `discovery-vanshika-${Date.now()}.mp4`;
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Vanshika');
