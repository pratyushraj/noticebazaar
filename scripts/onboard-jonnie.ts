...
async function main() {
  // Utilizing a clean placeholder email that can easily be updated when they log in to claim it
  const email = 'jonnieandoskyboy@gmail.com';
  const username = 'jonnieandoskyboy';
  const fullName = 'Jonnie&OskyBoy | Pet Creator 🐾';
  const password = 'CreatorArmour2026!'; // Temporary credential
  const reelUrl = 'https://www.instagram.com/reel/DYrGm1WpxjK/?igsh=c2t4YngxcjQwNTRk';

  console.log(`🚀 Starting Full Onboarding & Discovery Video Asset pipeline for Jonnie (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'Jonnie & Osky Boy 🐾',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Pet Care',
      instagram_followers: 29900,
      followers_count: 29900,
      engagement_rate: 7.4, // Exact official Meta Interaction Rate (7.4%)
      avg_views: 65000,      // Calculated average view metric based on 1.4M reach
      avg_likes_manual: 2200,
      avg_reel_views_manual: 65000,
      reel_price: 6000,     // Provided by user (₹6,000 for Reel)
      story_price: 1000,     // Estimated commercial standard for story
      starting_price: 6000,
      ...
      city: 'Dehradun',
      bio: '🐾 Jonnie & Osky Boy | Pet | Lifestyle | Reels | Based in Dehradun, Uttarakhand 📍 Dehradun, India',
      intro_line: 'Verified Pet & Lifestyle Creator from Dehradun with 29K followers and an amazing 7.4% Interaction Rate 🐾🐕',
      collab_intro_line: 'Jonnie & Osky Boy produces adorable dog adventures, premium pet grooming integration, healthy treats reviews, and lifestyle vlogs.',
      ...
      top_cities: ['Delhi', 'Bangalore', 'Mumbai', 'Kolkata'],
      audience_age_range: '18-24 (38.9%)',
      ...
      // Marketplace Intel
      deal_intelligence: {
        hookRate: 72.7, // 72.7% hook rate from Marketplace (extremely high outlier!)
        interactionRate: 7.4, // 7.4% from Marketplace
        accountsReached30d: '1.4M', // 1.4M from Marketplace
        accountsEngaged30d: '148.2K', // 148.2K from Marketplace
        viralPotential: 'Elite',
        demographicsRelevance: '91.2% India Concentrated'
      },
      collab_audience_fit_note: 'High 91.2% India concentration, heavily dense in key metros (Delhi, Bengaluru, Mumbai).',
      collab_engagement_confidence_note: 'Outstanding 7.4% interaction rate combined with a phenomenal 72.7% hook retention rate (very high viewer hooks).',
      collab_delivery_reliability_note: 'Excellent reach with over 1.4M monthly accounts reached and extremely high engagement rates.',
      collab_cta_trust_note: 'Outstanding conversion potential for premium pet food brands, interactive toys, grooming kits, health supplements, and pet-friendly travel stays.',
      ...
      deal_templates: [
        {
          ...
          price: 6000,
          budget: 6000,
          description: '1 Adorable Collab Reel featuring Jonnie & Osky Boy - High hook retention, interactive dog showcase, and lifestyle pet integration.',
          deliverables: [
            '1 Collab Reel (15-30s) featuring the dogs',
            'Elite hook rate retention focus',
            '1 Revision included'
          ]
        },
        {
          ...
          budget: 8000,
          isPopular: true,
          description: '1 Premium Reel + 2 Stories - Excellent for direct product conversions, accessory vlogs, and custom link tracking.',
          deliverables: [
        ...
        },
        {
          ...
          description: 'Barter collaboration - Only accepting premium pet foods, grooming spa kits, healthy treats, or high-value toys (minimum value ₹4,000+).',
          deliverables: [
            '1 Collab Reel or 2 Stories',
            'Product review showcase focus'
          ]
        }
      ]
    };
    ...
    const rawFileName = `raw-jonnie-${Date.now()}.mp4`;
    const rawFilePath = path.join(tempDir, rawFileName);
    
    const optFileName = `discovery-jonnie-${Date.now()}.mp4`;
    ...
    try {
      execSync(`ffmpeg -i "${rawFilePath}" -vcodec libx264 -profile:v main -level 3.1 -pix_fmt yuv420p -vf "scale='min(720,iw)':-2" -crf 28 -movflags +faststart -threads 0 -preset fast "${optFilePath}"`, { stdio: 'inherit' });
    } catch (error) {
    ...
    // Phase 8: Generate sitemap and links
    console.log(`[8/8] Public onboarding profile and dynamic storefront is now active for ${fullName}!`);
