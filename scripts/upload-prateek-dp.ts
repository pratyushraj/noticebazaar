...
async function main() {
  const profileId = '833282be-9af0-4149-8d44-a660cb309840';
  const username = 'prateek_matoria';
  const localPath = join(process.cwd(), 'public/creator-assets/prateek_avatar_cropped.jpg');
  const timestamp = Date.now();
  const storagePath = `${profileId}/profile-${timestamp}.jpg`;

  console.log(`📤 Uploading new cropped profile picture for @${username} (ID: ${profileId})...`);
  ...
  try {
    ...
    console.log('✅ Cropped avatar uploaded to storage successfully.');
