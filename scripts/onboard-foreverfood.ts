...
async function main() {
  const email = 'poonamcreater10@gmail.com';
  const username = 'foreverfood.p';
  const fullName = 'Punam Priyadarshi | Homemade Recipes 👩‍🍳';
  const password = 'CreatorArmour2026!'; // Temporary credential
  const reelUrl = 'https://www.instagram.com/reel/DMF8MqHSMBG/?igsh=bnRmYnYzaHVvcGc5';

  console.log(`🚀 Starting Full Onboarding & Discovery Video Asset pipeline for Punam (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'Punam Priyadarshi 👩‍🍳',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Food & Lifestyle',
      instagram_followers: 34200,
      followers_count: 34200,
      engagement_rate: 6.0, // Exact official Meta Interaction Rate (6.0%)
      avg_views: 75000,      // Calculated average view metric based on 1.6M reach and high interaction
      avg_likes_manual: 3500,
      avg_reel_views_manual: 75000,
      reel_price: 12000,     // Provided by user (₹12,000 for Reel)
      story_price: 1800,     // Estimated commercial standard for story
      ...
      city: 'Patna',
      bio: 'Homemade recipes 👩‍🍳 | Food ideas 🍲 | DM for collaborations 📍 Patna, Bihar, India',
      intro_line: 'Verified Homemade Recipes & Food Creator from Patna with 34K followers and a strong 6% Interaction Rate 🍲✨',
      collab_intro_line: 'Punam Priyadarshi produces aesthetic food recipe reels, gourmet home-kitchen reviews, and cooking/grocery D2C brand integrations.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),

      // Audience Demographics Snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 82.6, men: 17.4 }, // Outlier female high density
      top_cities: ['Delhi', 'Mumbai', 'Ahmedabad', 'Bangalore'],
      audience_age_range: '25-34 (50.8%)',
      ...
      // Marketplace Intel
      deal_intelligence: {
        hookRate: 55.8, // 55.8% from Marketplace
        interactionRate: 6.0, // 6.0% from Marketplace
        accountsReached30d: '1.6M', // 1.6M from Marketplace
        accountsEngaged30d: '147.6K', // 147.6K from Marketplace
        viralPotential: 'Very High',
        demographicsRelevance: '97.2% India Concentrated'
      },
      collab_audience_fit_note: 'Exceptional 82.6% Female concentration—prime target demographic for D2C kitchenware, healthy oils, organic foods, spreads, and parenting services.',
      collab_engagement_confidence_note: 'Strong 6.0% interaction rate backed by an excellent 55.8% hook rate (both are market outperformers).',
      collab_delivery_reliability_note: 'Proven regional reach with over 1.6M monthly impressions and active comment sections.',
      collab_cta_trust_note: 'Outstanding conversion potential for cooking ingredients, spices, cookware, quick-commerce grocery services, and health/wellness foods.',
      ...
      deal_templates: [
        {
          ...
          price: 12000,
          budget: 12000,
          description: '1 High-quality Collab Reel featuring Punam Priyadarshi - High hook retention, authentic home-kitchen recipe review, and regional reach.',
          deliverables: [
            '1 Collab Reel (30-60s) with recipe integration',
            'Hook rate optimization focus',
            '1 Revision included'
          ]
        },
        {
          ...
          budget: 15000,
          isPopular: true,
          description: '1 Premium Dedicated Reel + 2 Stories - Excellent for driving direct brand sales, organic spice reviews, and gourmet quick-commerce conversions.',
          deliverables: [
        ...
        },
        {
          ...
          description: 'Barter collaboration - Only accepting premium kitchen gadgets, cookware, organic ingredients, and health food sets (minimum value ₹8,000+).',
          deliverables: [
            '1 Dedicated Collab Reel or 2 Stories',
    ...
    const rawFileName = `raw-foreverfood-${Date.now()}.mp4`;
    const rawFilePath = path.join(tempDir, rawFileName);
    
    const optFileName = `discovery-foreverfood-${Date.now()}.mp4`;
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Punam');
