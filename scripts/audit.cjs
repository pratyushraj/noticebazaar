...
const TUNNEL = 'https://sorts-areas-platinum-quarterly.trycloudflare.com';;;;
...
const doLogin = async (page, creds, label) => {
  try {
    ...
    // Wait for the actual input to be rendered by React
    ...
    log(`${label} login ERROR: ${e.message.substring(0, 100)}`);
    return false;
  }
};
...
(async () => {
  ...
  creatorPage.on('console', msg => {
    if (msg.type() === 'error') log(`CREATOR CONSOLE ERROR: ${msg.text().substring(0, 80)}`);
  });
  
  if (await doLogin(creatorPage, CREATOR, 'Creator')) {
    await creatorPage.goto(`${TUNNEL}/creator-dashboard?tab=deals`, { timeout: 20000, waitUntil: 'domcontentloaded' });
    ...
    await creatorPage.goto(`${TUNNEL}/creator-dashboard?tab=account`, { timeout: 20000, waitUntil: 'domcontentloaded' });
    await creatorPage.waitForTimeout(3000);
  ...
  brandPage.on('console', msg => {
    if (msg.type() === 'error') log(`BRAND CONSOLE ERROR: ${msg.text().substring(0, 80)}`);
  });
  
  if (await doLogin(brandPage, BRAND, 'Brand')) {
    await brandPage.goto(`${TUNNEL}/brand-dashboard?tab=collabs`, { timeout: 20000, waitUntil: 'domcontentloaded' });
    ...
    log(`Collabs: need_your_review=${hasNeedsReview}, all_caught_up=${hasAllCaughtUp} ${hasNeedsReview && hasAllCaughtUp ? '⚠️ CONTRADICTION' : ''}`);
    
    await brandPage.goto(`${TUNNEL}/brand-dashboard?tab=payments`, { timeout: 20000, waitUntil: 'domcontentloaded' });
    await brandPage.waitForTimeout(3000);
