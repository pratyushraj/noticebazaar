...
async function main() {
  const email = 'Ky641688@gmail.com';
  const username = 'kajukatlivlogs';
  const fullName = 'Kajal Yadav';
  const password = 'CreatorArmourKaju2026!'; // Temporary password
  const optimizedVideoPath = path.join(process.cwd(), 'scratch', 'kaju_optimized.mp4');

  console.log(`🚀 Starting Onboarding & Asset synchronization pipeline for Kajal Yadav (@${username})...`);
  ...
  try {
    ...
    const profileUpdate = {
      ...
      business_name: 'Kajal Yadav | Aesthetic Lifestyle, Fashion & Beauty Curation ✨',
      role: 'creator',
      onboarding_complete: true,
      creator_category: 'Fashion & Beauty',
      instagram_followers: 7804,
      followers_count: 7804,
      engagement_rate: 5.6, // Official Meta Interaction Rate from screenshot
      avg_views: 45000,
      avg_reel_views_manual: 45000,
      reel_price: 4000,      // ₹4,000 paid starting rate (1 Reel + 2 Stories)
      story_price: 1500,      // Standalone story rate
      starting_price: 4000,
      barter_min_value: 3000, // Barter minimum value
      ...
      city: 'Patna',
      bio: '✨ Model & Digital Creator | Specializing in Aesthetic & Luxury Content 📍 Patna based. Curation of professional vlogs, cinematic cafe reviews, makeup routines, and high-fashion aesthetics 🌸',
      intro_line: 'Verified Fashion, Beauty & Lifestyle Creator from Patna with 7.8K followers and 5.6% Interaction Rate ✨',
      collab_intro_line: 'Kajal Yadav features high-aesthetic personal styling lookbooks, professional cafe/lifestyle vlogs, and premium cinematic brand showcases.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),
 
      // Demographic snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 87.9, men: 12.1 }, // Spectacular 87.9% Female split!
      top_cities: ['Patna', 'Delhi', 'Supaul', 'Mumbai'],
      audience_age_range: '18-24 (51.4%)',
      ...
      // Marketplace Intel
      deal_intelligence: {
        hookRate: 50.3,       // Strong 50.3% hook rate from screenshot
        interactionRate: 5.6,
        accountsReached30d: '389.5K', // From screenshot
        accountsEngaged30d: '33.8K', // From screenshot
        viralPotential: 'Medium',
        demographicsRelevance: '98.8% India Concentrated'
      },
      collab_audience_fit_note: 'Outstanding 87.9% Female engaged audience concentration—highly optimal for premium skincare labels, cosmetics, beauty salons, aesthetic clothing lines, and cafe experiences.',
      collab_engagement_confidence_note: 'Strong visual styling combined with a solid 5.6% Interaction Rate, 50.3% Hook Rate, and 33.8K highly engaged users.',
      collab_delivery_reliability_note: 'Patna-based UGC and lifestyle creator specializing in professional cinematic reviews and high-fashion aesthetics with swift, reliable execution.',
      collab_cta_trust_note: 'Strong conversion potential for cafes, fashion lookbooks, makeup brands, cosmetic enhancements, and luxury experience packages.',
    ...
    const videoFileName = `discovery-kaju-${Date.now()}.mp4`;
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Kajal');
