...
async function main() {
  const email = process.argv[2] || 'shivyayadav93@gmail.com';
  const username = 'shivya_yadav93';
  const fullName = 'Shivya Yadav';
  const password = 'CreatorArmourShivya2026!'; // Temporary password
  const tempDir = path.join(process.cwd(), 'temp');
  const optimizedVideoPath = path.join(tempDir, 'shivya_optimized.mp4');

  console.log(`🚀 Starting Onboarding & Asset synchronization pipeline for Shivya Yadav (@${username})...`);
  ...
  try {
    ...
    // Phase 2: Fetch and Upload Avatar
    console.log('[2/8] Fetching public high-resolution avatar via unavatar proxy...');
    ...
    const profileUpdate = {
      ...
      business_name: 'Shivya Yadav | Beauty, Fashion & Lifestyle Curation ✨',
      role: 'creator',
      ...
      engagement_rate: 5.8,
      avg_views: 25000,
      avg_reel_views_manual: 25000,
      reel_price: 5000,     // ₹5,000 starting reel rate
      story_price: 1500,     // Standard story price
      starting_price: 5000,
      ...
      city: 'Muzaffarpur',
      bio: 'Radhe Radhe🌺 📍Bihar (Muzaffarpur) • Creator sharing beauty/lifestyle curation and Patna-focused aesthetic storytelling ✨',
      intro_line: 'Verified Beauty & Lifestyle Creator from Muzaffarpur, Bihar with 15.1K followers and 5.8% Interaction Rate 💫🌸',
      collab_intro_line: 'Shivya Yadav features high-engagement beauty lookbooks, aesthetic cosmetic curation, creative product storytelling, and local Bihar collaborations.',
      last_instagram_sync: new Date().toISOString(),
      updated_at: new Date().toISOString(),

      // Demographic snapshot from Meta Creator Marketplace
      audience_gender_split: { women: 89.6, men: 10.4 }, // EXACT split from Marketplace
      top_cities: ['Delhi', 'Patna', 'Mumbai', 'Bengaluru'], // EXACT top cities from Marketplace
      audience_age_range: '18-24 (80.7%)', // EXACT age range from Marketplace
      ...
      // Marketplace Intel
      deal_intelligence: {
        hookRate: 58.4,       // EXACT hook rate: 58.4%
        interactionRate: 5.8, // EXACT interaction rate: 5.8%
        accountsReached30d: '375.3K', // EXACT reached: 375.3K
        accountsEngaged30d: '24.8K', // EXACT engaged: 24.8K
        viralPotential: 'High',
        demographicsRelevance: '89.6% Female concentration, 97.3% India geolocated'
      },
      collab_audience_fit_note: 'Fantastic 89.6% Female engaged audience concentration—highly optimal for cosmetic/skincare products, beauty brands, local Patna cafes/salons, and retail fashion collections.',
      collab_engagement_confidence_note: 'High-impact beauty aesthetics combined with an exceptionally strong 58.4% Hook Rate and solid 375.3K monthly reach.',
      collab_delivery_reliability_note: 'Highly active beauty and lifestyle creator with professional DM responses, very high execution standards, and rich curation.',
      collab_cta_trust_note: 'Excellent CTR and conversions for visual cosmetics, self-care routines, wellness products, and modern daily aesthetics.',

      // Custom Standardized Packages (Starting from 5k)
      deal_templates: [
        {
          id: 'starter_collab',
          name: '🚀 Starter Collab',
    ...
    const videoFileName = `discovery-shivya-${Date.now()}.mp4`;
    ...
    const htmlBody = getEmailLayout(ctaButton, 'Shivya');
