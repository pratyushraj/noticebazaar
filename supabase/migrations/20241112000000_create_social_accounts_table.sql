...
-- Create social_accounts table (extended version with user_id and access tokens)
-- Uses IF NOT EXISTS to be safe when earlier migration already created this table
...
-- Add user_id column if it doesn't exist (in case created by earlier migration)
ALTER TABLE public.social_accounts ADD COLUMN IF NOT EXISTS user_id uuid REFERENCES public.profiles(id) ON DELETE CASCADE;
ALTER TABLE public.social_accounts ADD COLUMN IF NOT EXISTS access_token bytea;
ALTER TABLE public.social_accounts ADD COLUMN IF NOT EXISTS refresh_token bytea;
...
ALTER TABLE public.social_accounts ADD COLUMN IF NOT EXISTS verified boolean DEFAULT false;
ALTER TABLE public.social_accounts ADD COLUMN IF NOT EXISTS connected_at timestamp with time zone DEFAULT timezone('utc', now());
ALTER TABLE public.social_accounts ADD COLUMN IF NOT EXISTS updated_at timestamp with time zone DEFAULT timezone('utc', now());

-- Only create unique index if user_id exists and is not null
DO $$
BEGIN
  IF EXISTS (
