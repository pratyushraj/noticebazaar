...
-- Enable RLS on storage.objects is managed by Supabase platform (superuser only)
-- Policies below will still apply as storage RLS is always on in Supabase
...
-- Comments on storage.buckets/objects require owner privilege, skipping
